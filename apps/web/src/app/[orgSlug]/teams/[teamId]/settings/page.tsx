...
function GeneralPanel({ team, workspaceSlug, canManage, onUpdate }: {
  ...
  const save = async () => {
    ...
    setSaved(false);
    try {
      const { error: err } = await api.put(`/api/workspaces/${workspaceSlug}/teams/${team.id}`, form);
      if (err) {
        throw new Error(err || 'Error guardando');
      }
      onUpdate({ ...team, ...form });
      setSaved(true);
...
export default function TeamSettingsPage() {
  ...
  const fetchTeam = useCallback(async () => {
    ...
    setError('');
    try {
      const { data, error: err } = await api.get<{ team: TeamData }>(`/api/workspaces/${workspace.slug}/teams/${teamId}`);
      if (err || !data) throw new Error('Equipo no encontrado');
      setTeam(data.team);
    } catch (err) {
