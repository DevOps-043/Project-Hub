...
/**
 * Shell compartido de las 7 pestañas de un equipo. Antes cada subpágina
 * (Tareas/Proyectos/Ciclos/Miembros/Configuración) repetía su propio fetch
 * de equipo y su propio header/back-link — con destinos de "volver"
 * distintos e incorrectos en cada una. Este layout hace el fetch una sola
 * vez, expone el resultado vía TeamPanelContext y renderiza el header +
 * TeamTabBar de forma idéntica en las 7 vistas.
 */
export default function TeamPanelLayout({ children }: { children: React.ReactNode }) {
  ...
  const refreshTeam = useCallback(async () => {
    setError('');
    try {
      const { data, error: apiError } = await api.get<{ team: TeamPanelTeam }>(`/api/workspaces/${orgSlug}/teams/${teamId}`);
  ...
  return (
    <TeamPanelContext.Provider
      value={{
        team, loading, error, refreshTeam, panelBase, workspaceSlug: orgSlug, teamId,
        canManage: permissions.manageTeams, canDelete: permissions.manageWorkspace,
      }}
    >
