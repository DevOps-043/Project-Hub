...
interface MemberTask {
  ...
  priority?: { color?: string | null } | null;
  status?: { name?: string | null; status_type?: string | null; color?: string | null } | null;
}
...
/**
 * Perfil de miembro de equipo. Compartido entre /admin/teams/[teamId]/members/[memberId]
 * y /[orgSlug]/admin/teams/[teamId]/members/[memberId], que eran implementaciones
 * duplicadas idénticas salvo por el prefijo de ruta del breadcrumb.
 */
export default function MemberProfileView() {
  const params = useParams();
  const { teamId, memberId, orgSlug } = params as { teamId: string; memberId: string; orgSlug?: string };
  ...
  const membersListHref = orgSlug ? `/${orgSlug}/admin/teams/${teamId}/members` : `/admin/teams/${teamId}/members`;
  ...
  const fetchMemberDetails = useCallback(async () => {
    try {
      const { data: jsonData, error } = await api.get<MemberProfileData>(`/api/admin/teams/${teamId}/members/${memberId}`);
      if (error) throw new Error('Failed to fetch member details');
      setData(jsonData);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const completedTasks = tasks.filter((t) => t.status?.status_type === 'done' || t.status?.name === 'completed').length;
    ...
    <div className="min-h-screen" style={{ backgroundColor: colors.bgPrimary }}>
      ...
      <header className="relative z-10 max-w-7xl mx-auto px-6 pt-8 pb-6">
         <div className="flex items-center gap-2 mb-4 text-sm" style={{ color: colors.textMuted }}>
            <Link href={membersListHref} className="hover:text-white transition-colors">Miembros</Link>
            <span>/</span>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 pb-12">
        ...
        >
            <div className="relative">
                ...
                >
                    {user.avatar_url ? (
                        <img src={user.avatar_url} alt={user.display_name || user.first_name || 'Avatar'} className="w-full h-full object-cover rounded-3xl" />
                    ) : (user.first_name?.[0] || 'U')}
                </div>
                    ...
                    {tasks.length > 0 ? (
                        tasks.map((task) => (
                             <div key={task.issue_id} className="p-4 rounded-xl border flex items-center gap-4 group transition-all hover:shadow-md hover:border-blue-500/30"
                                ...
                                <div className={`w-3 h-3 rounded-full mt-1 shrink-0`} style={{ backgroundColor: task.priority?.color || '#ccc' }} />
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-4 text-xs opacity-60" style={{ color: colors.textMuted }}>
                                        <span className="flex items-center gap-1">
                                            <span className="w-2 h-2 rounded-full" style={{ backgroundColor: task.status?.color ?? undefined }}></span>
                                            {task.status?.name}
                                        </span>
...
function StatCard({ title, value, icon, color, isDark, colors }: StatCardProps) {
    return (
        <motion.div
            whileHover={{ y: -2 }}
