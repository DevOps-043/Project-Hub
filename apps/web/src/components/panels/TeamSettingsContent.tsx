...
// --- Danger Zone Panel ---
function DangerPanel({ team, workspaceSlug, canManage, canDelete, panelBase }: {
  team: TeamData;
  workspaceSlug: string;
  canManage: boolean;
  canDelete: boolean;
  ...
  if (error || !team) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <button onClick={() => router.push(`${panelBase}/teams`)}
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'danger' && (
              <motion.div key="danger" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}>
                <DangerPanel team={team} workspaceSlug={workspace.slug} canManage={canManage} canDelete={permissions.manageWorkspace} panelBase={panelBase} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
