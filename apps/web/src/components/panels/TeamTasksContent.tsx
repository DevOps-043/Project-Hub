...
const PriorityIcon = ({ level, color }: { level: number; color: string }) => {
  ...
  return (
    ...
    >
      ...
      {level === 2 && <span className="text-[10px] font-bold">&uarr;</span>}
      {level === 3 && <span className="text-[10px] font-bold">=</span>}
      {level === 4 && <span className="text-[10px] font-bold">&darr;</span>}
    </div>
  );
};
                      ...
                      {statusIssues.length === 0 ? (
                        ...
                        <div className="space-y-0.5 pb-2">
                          {statusIssues.map(issue => (
                            <Link
                              key={issue.issue_id}
                              href={`${panelBase}/teams/${teamId}/tasks/${issue.issue_id}`}
